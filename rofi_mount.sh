...
remotemnt() {
    ...
    if [ ! -d "$mntpoint" ]; then mkdir "$mntpoint"; fi && \
        "$HOME/coding/hidden/mount/$chosen.sh" "$mntpoint" && \
        notify-send -i "$HOME/coding/shell/icons/usb.png" "Mount Remote" "$chosen mounted to $mntpoint"
}
...
usbmnt() {
    ...
    "vfat") sudo -A mount -t vfat "$chosen" "$mntpoint" -o rw,umask=0000 && \
        notify-send -i "$HOME/coding/shell/icons/usb.png" "Mount $parttype USB" "$chosen mounted to $mntpoint" ;;
    *)
        sudo -A mount "$chosen" "$mntpoint" && \
            notify-send -i "$HOME/coding/shell/icons/usb.png" "Mount $parttype USB" "$chosen mounted to $mntpoint"
        user="$(whoami)"
...
androidmnt() {
    ...
    mntpoint="$HOME"/mount/$chosen
    if [ ! -d "$mntpoint" ]; then mkdir "$mntpoint"; fi && \
        simple-mtpfs --device "$chosen" "$mntpoint" && \
        notify-send -i "$HOME/coding/shell/icons/usb.png" "Mount Android" "$chosen mounted to $mntpoint"
}
