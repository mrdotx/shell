...
seconddisplay() {
    ...
    if [ "$mirror" = "yes" ]; then
        external=$(echo "$displays" | rofi -dmenu -i -p "Optimize resolution for:")
        ...
        primary=$(echo "$displays" | rofi -dmenu -i -p "Select primary display:")
        secondary=$(echo "$displays" | grep -v "$primary")
        direction=$(printf "right\\nleft" | rofi -dmenu -i -p "What side of $primary should $secondary be on?")
