...
printer_ser() {
    if [ "$printer_stat" != "active" ]; then
        ...
        sudo -A systemctl stop org.cups.cupsd.service && \
            notify-send -i "$HOME/coding/shell/icons/service.png" "Service" "Printer stopped!" && exit 0
    fi
}
...
avahi_ser() {
    if [ "$avahi_ser_stat" != "active" ]; then
        ...
        sudo -A systemctl stop avahi-daemon.service && \
            ...
            notify-send -i "$HOME/coding/shell/icons/service.png" "Service" "Avahi stopped!" && exit 0
    fi
}
...
bluetooth_ser() {
    if [ "$bluetooth_stat" != "active" ]; then
        ...
        sudo -A systemctl stop bluetooth.service && \
            notify-send -i "$HOME/coding/shell/icons/service.png" "Service" "Bluetooth stopped!" && exit 0
    fi
}
...
firewall_ser() {
    if [ "$firewall_stat" != "active" ]; then
        ...
        sudo -A systemctl stop ufw.service && \
            notify-send -i "$HOME/coding/shell/icons/service.png" "Service" "Firewall stopped!" && exit 0
    fi
}
